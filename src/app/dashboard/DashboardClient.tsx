...
import {
  ...
  Pencil, Trash2, Home, Utensils, Car, Gamepad2, Lightbulb, Equal, Hash, Percent, Share2,
...
export default function DashboardClient(props: Props) {
  ...
  function openEditModal(exp: Expense) {
    ...
    // Find group members for this expense's group
    const g = groupsWithBalances.find(gr => gr.id === exp.group.id)
    if (g) {
      setEditParticipants(new Set(g.members.map(m => m.id)))
    }
          ...
          <Dialog.Content bg="bg.panel" borderRadius="card" maxW={{ base: "100%", md: "480px" }} maxH="85vh" overflowY="auto" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleUpdateExpense} style={{ padding: "24px 24px 16px" }}>
                ...
                <NativeSelect.Root size="lg">
                  <NativeSelect.Field value={editPaidBy} onChange={e => setEditPaidBy(e.target.value)}>
                    {(() => {
                      ...
                      return g ? g.members.map(m => (
                        <option key={m.id} value={m.id}>{m.id === user.id ? "Você" : m.name.split(" ")[0]}</option>
                      )) : null
                    })()}
                  </NativeSelect.Field>
                  <NativeSelect.Indicator />
              ...
              {(() => {
                ...
                const members = (groupsWithBalances.find(gr => gr.id === editingExpense?.group.id)?.members || [])
                const participantsList = members.filter(m => editParticipants.has(m.id))
                    ...
                    <Flex direction="column" gap={2}>
                      {editSplitType === "EQUAL" && preview.map(p => (
                        <Flex key={p.userId} justify="space-between" fontSize="sm">
                          <Text>{members.find(m => m.id === p.userId)?.name?.split(" ")[0] || "Alguém"}</Text>
                      ...
                      {editSplitType === "FIXED" && participantsList.map(p => (
                        <Flex key={p.id} align="center" gap={3}>
                          <Text fontSize="sm" w="80px" truncate>{members.find(m => m.id === p.id)?.name?.split(" ")[0] || "Alguém"}</Text>
